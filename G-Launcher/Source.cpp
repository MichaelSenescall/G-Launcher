...
const std::string version_current = "v1.2";
...
int WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	for (int i = 1; i < argc; i++)
	{
		...
		Json::Value jsonData;
		file >> jsonData;
		std::string url = jsonData["url"].asString();

		//ShellExecute(0, 0, ConvertUtf8ToWide(url).c_str(), 0, 0, SW_SHOW);
		if ((int)ShellExecute(NULL, L"open", ConvertUtf8ToWide(url).c_str(), NULL, NULL, SW_SHOWNORMAL) <= 32)
		{
			// Do something if URL fails to open
		}
	}
